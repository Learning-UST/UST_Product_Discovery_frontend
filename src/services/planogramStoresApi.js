...
const DEFAULT_STORE_REQUEST_TIMEOUT_MS = 1800
...
export const fetchPlanogramStores = async ({ identity } = {}) => {
    ...
    // Backward-compatible fallback in case global scan returns nothing.
    ...
    writeStoresCache(toArray(stores))

    return {
        stores: toArray(stores).map(mapStoreForUi),
        hasIdentity: true,
    }
}
